...
func (c *Config) RegisterConsulService() error {
	...
	port, err := strconv.ParseInt(c.EventStoreServiceRpcPort, 10, 64)
	if err != nil {
	...
	if err = client.Agent().ServiceRegister(&api.AgentServiceRegistration{
		...
		Name:    c.EventStoreServiceServiceName,
		Address: c.EventStoreServiceRpcHost,
		Port:    int(port),
		Tags:    []string{"service", "rabbitmq-client"},
		Check: &api.AgentServiceCheck{
			GRPC:                           fmt.Sprintf("%s:%s", c.EventStoreServiceRpcHost, c.EventStoreServiceRpcPort),
			GRPCUseTLS:                     false,
