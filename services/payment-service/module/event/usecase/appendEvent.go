...
func (u *eventUseCase) AppendEvent(ctx context.Context, request *pbEvent.Event) error {
	...
	if err = u.kafkaInfrastructure.PublishWithSchema(ctx, config.Get().BrokerKafkaTopicConnectorSinkMongoEvent.Payment, request.XId, kafka.PROTOBUF_SCHEMA, request); err != nil {
		u.logger.Error("eventUseCase.AppendEvent", zap.String("requestId", request.SagaId), zap.Error(err))
		return status.Error(codes.Internal, "internal server error")
	}
	return nil
