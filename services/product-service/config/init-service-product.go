...
type ConfigServiceProduct struct {
	ServiceName string
...
func DefaultConfigServiceProduct() *ConfigServiceProduct {
	return &ConfigServiceProduct{
		ServiceName:    "",
		RpcHost:        "",
		RpcPort:        "",
		HttpHost:       "",
		HttpPort:       "",
		MetricHttpPort: "",
		keyPrefix:      "%s/services/product/%s",
	}
}

func (c *Config) withServiceProduct(kv *api.KV) *Config {
	c.ConfigServiceProduct = DefaultConfigServiceProduct().WithConsulClient(c.Env, kv)
	return c
}

func (c *ConfigServiceProduct) WithConsulClient(env string, kv *api.KV) *ConfigServiceProduct {
