...
public class OpenTelemetryOutboundKafka<K, V> implements ProducerInterceptor<K, V> {
    @Override
    public ProducerRecord<K, V> onSend(ProducerRecord<K, V> record) {
        record.headers().add(TRACEPARENT_CONTEXT_KEY, MDC.get(TRACEPARENT_CONTEXT_KEY).getBytes());
        return record;
    }
