...
func (c *DatabaseMongo) WithConsulClient(env string, kv *api.KV) *DatabaseMongo {
	...
	if err != nil {
		log.Fatalf("SetConfig | could not get MONGO_DATABASE_NAME/EVENT_STORE from consul: %v", err)
	}
	if pair == nil {
		log.Fatal("SetConfig | Consul | MONGO_DATABASE_NAME/EVENT_STORE is required")
	}
