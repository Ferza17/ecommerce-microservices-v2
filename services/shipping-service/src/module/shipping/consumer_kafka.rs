...
impl Consumer {
    ...
    pub async fn consume_confirm_snapshot_shippings_shipping_created(
        &self,
        message: OwnedMessage,
    ) -> Result<(), anyhow::Error> {
        let mut request_id = String::new();
        ...
        let mut request = ReserveEvent::default();
        match message.payload() {
            None => {
                info!("no payload found");
                Err(anyhow::Error::msg("no payload found"))?
            ...
            Some(p) => {
                request = match ReserveEvent::decode(&*p) {
                    Ok(v) => v,
                    Err(err) => {
                        error!(
                            "[consume_confirm_snapshot_shippings_shipping_created] consume_confirm_snapshot_shippings_shipping_created : {}",
                            err
                        );
                        return Err(anyhow::Error::msg(err.to_string()));
                    }
        ...
        match self
            .shipping_use_case
            .confirm_create_shipping(request_id, token, tonic::Request::new(request))
            .await
        {
            Ok(response) => {
                info!(
    ...
    pub async fn consume_compensate_snapshot_shippings_shipping_created(
        &self,
        message: OwnedMessage,
    ) -> Result<(), anyhow::Error> {
        let mut request_id = String::new();
        ...
        match self
            .shipping_use_case
            .compensate_create_shipping(request_id, token, tonic::Request::new(request))
            .await
        {
            Ok(response) => {
                info!(
                    "[consume_compensate_snapshot_shippings_shipping_created] response: {:?}",
                    response
                );
            }
            Err(err) => {
                error!(
                    "[consume_compensate_snapshot_shippings_shipping_created] error: {:?}",
                    err
                );
                return Err(anyhow::Error::msg(err.to_string()));
            }
