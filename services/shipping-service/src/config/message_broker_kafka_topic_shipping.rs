...
impl MessageBrokerKafkaTopicShipping{
    pub async fn with_consul_client(
        ...
        self.snapshot_shippings_shipping_created =
            crate::config::config::get_kv(client, format!("{}/broker/kafka/TOPICS/SHIPPING/SHIPPING_CREATED", env)).await;
        self.snapshot_shippings_shipping_updated =
            crate::config::config::get_kv(client, format!("{}/broker/kafka/TOPICS/SHIPPING/SHIPPING_UPDATED", env)).await;
        Ok(self.clone())
    }
}