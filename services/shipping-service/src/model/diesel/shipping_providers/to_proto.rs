...
pub fn shipping_provider_to_proto(shipping_provider: ShippingProviders) -> ProtoShippingProvider {
    ProtoShippingProvider {
        ...
        created_at: Option::from(prost_wkt_types::Timestamp::from(shipping_provider.created_at)),
        updated_at: Option::from(prost_wkt_types::Timestamp::from(shipping_provider.updated_at)),
        discarded_at: shipping_provider.discarded_at.map(|dt| prost_wkt_types::Timestamp::from(dt)),
    }
}
