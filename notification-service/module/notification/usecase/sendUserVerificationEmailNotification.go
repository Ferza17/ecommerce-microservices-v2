...
func (u *notificationUseCase) SendUserVerificationEmailNotification(ctx context.Context, requestId string, req *pb.SendUserVerificationEmailNotificationRequest) error {
	var (
		err        error
		eventStore = &pb.EventStore{
			RequestId:     requestId,
			Service:       enum.NotificationService.String(),
			EventType:     enum.NOTIFICATION_USER_CREATED.String(),
	...
	if err = u.mailHogInfrastructure.SendMail(&mailHogInfrastructure.Mailer{
		...
		u.logger.Error(fmt.Sprintf("error sending email: %s", err.Error()))
		return err
	}
