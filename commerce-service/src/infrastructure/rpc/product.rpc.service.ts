...
export class ProductRpcService implements OnModuleInit {
  ...
  constructor(@Inject(Service.ProductService.toString()) private client: ClientGrpc) {
  }
  ...
  async findProductById(requestId: string, req: FindProductByIdRequest): Promise<Product> {
    try {
