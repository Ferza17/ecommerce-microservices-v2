...
import (
	...
	"github.com/ferza17/ecommerce-microservices-v2/api-gateway/server/graphql/middleware"
	"github.com/ferza17/ecommerce-microservices-v2/api-gateway/server/graphql/resolver"
...
func (srv *Server) Serve() {
	...
	if err != nil {
		srv.bootstrap.Logger.Error(fmt.Sprintf("error starting http server: %v", err))
		return
	}
}
...
func (srv *Server) Close() error {
	if err := srv.httpServer.Close(); err != nil {
		srv.bootstrap.Logger.Error(fmt.Sprintf("error closing http server: %v", err))
		return err
	}
	return nil
}
