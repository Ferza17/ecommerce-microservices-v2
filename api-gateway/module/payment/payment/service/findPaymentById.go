...
func (s *paymentService) FindPaymentById(ctx context.Context, requestId string, request *paymentRpc.FindPaymentByIdRequest) (*paymentRpc.Payment, error) {
	...
	resp, err := s.cb.Execute(func() (interface{}, error) {
		...
		return s.svc.FindPaymentById(ctx, request)
	})
